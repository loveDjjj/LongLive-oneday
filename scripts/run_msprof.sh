...
# Deployment settings. SP/DP and the dedicated VAE device come from the YAML.
...
"${GENERATION_ENV}/bin/python" scripts/resolve_inference_config.py msprof \
...
# JSON dictionaries do not have a stable shell representation. Read profiler
# fields directly with a small structured lookup.
json_nested() {
  "${GENERATION_ENV}/bin/python" -c \
...
mkdir -p "${video_dir}" "${profile_dir}" "${analysis_dir}"
...
if [[ "${recover_parse}" == "true" || \
  ...
  recovery_log="${run_dir}/profiling/recovery.log"

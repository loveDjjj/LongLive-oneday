...
export VBENCH_CACHE_DIR="${VBENCH_CACHE_DIR:-/path/to/vbench_models}"
...
CANN_ENV_SCRIPT="${CANN_ENV_SCRIPT:-/path/to/cann-8.5/Ascend/cann-8.5.0/set_env.sh}"
...
if ! NPU_IMPORT_ERROR="$(python -c 'import torch; import torch_npu; print(torch_npu.npu.device_count())' 2>&1)"; then
  ...
  echo "  source /path/to/cann-8.5/Ascend/cann-8.5.0/set_env.sh" >&2

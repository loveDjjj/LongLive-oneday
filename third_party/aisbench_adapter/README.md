...
启动器会加载 `/path/to/cann-8.5/Ascend/cann-8.5.0/set_env.sh`，将 `$CONDA_PREFIX/lib` 加到 `LD_LIBRARY_PATH` 前部，并在启动 AISBench 前检查 `torch_npu` 与 `decord`。这些检查用于避免缺少 `libhccl.so` 或错误加载旧版系统 C++ runtime。

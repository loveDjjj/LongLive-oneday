...
class DMD(SelfForcingModel):
    ...
    def _compute_kl_grad(
        ...
        if normalization:
            ...
            if F % self.num_frame_per_block != 0:
                raise ValueError(
                    f"Generated frames ({F}) must be divisible by block size "
                    f"({self.num_frame_per_block})"
                )
            p_real_blocks = p_real.view(
                B, F // self.num_frame_per_block, self.num_frame_per_block, C, H, W
            )
            normalizer = torch.abs(p_real_blocks).mean(
    ...
    def critic_loss(
        ...
        with torch.no_grad():
            ...
            sampled_noise = torch.randn(
                [image_or_video_shape[0], num_gen_frames, *image_or_video_shape[2:]],
                device=self.device, dtype=self.dtype)
            generated_image, _, _, _ = self._run_generator(
                image_or_video_shape=image_or_video_shape,
                conditional_dict=conditional_dict,
                slice_last_frames=slice_last_frames,
                noise=sampled_noise,
